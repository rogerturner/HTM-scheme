...
(define (tm-get-active-cols tm)           ;; TM -> (listof ColX)
  (let ((cells (list->vector (tm-active-cells tm))))
...
(define (hello-tm)
  (let* ( (format-row (lambda (x) (vector-fold-left     ;; (vectorof Number) -> String
    ...
    (for-each display `("Training: 10 repetitions of sdrs for \"ABCDE\"" #\newline))
...
(display "Enter \"(hello-tm)\" or \"(tm-high-order)\"") (newline)
