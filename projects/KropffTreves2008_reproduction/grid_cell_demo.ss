  ...
  (let ((max-rotation (fl/ (fl* 2.0 pi) 20.0)))
    ...
    (let ((new-position
      ...
      (cond
        ...
        [ else
            (env-angle-set! e (fl+ (env-angle e) pi))
            (env-seed-set! e (random-seed))
...
(define (_main args)                     ;; {KWarg} ->
  (let* (
      ...
      [enc-num-samples (argument 'enc-num-samples  4)]
      [gc-num-samples  (argument 'gc-num-samples   5)]
      ;; Setup
      [env        (make-env 256)]
  ...
  (let ((file "HTM-scheme/projects/KropffTreves2008_reproduction/experiment.data"))
    ...
    (define (receptive-field bitx step proc)  ;; Nat (Coord Coord -> SDR) -> {(Vectorof Coord)}
    ...
    (gc:reset gcm)
    (let* (
        [enc-samples (vector->list
          (vector-sample (build-vector (ce:ce-n enc) id) enc-num-samples))]
