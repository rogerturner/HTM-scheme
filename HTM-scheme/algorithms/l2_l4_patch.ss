    ...
    (lambda (new)
      (lambda (ncc cc ncl4pop nib bis cp-overrides attm-overrides)
        (let* (
          ...
          (when (l2:cp-same-col-only (vector-ref l2s 0))
    ...
    (threaded-vector-for-each ifge       ;; thread per cortical column
      (lambda (l2 l4 feature location ccx)
        (l4:compute l4 
          ...
          learn)
        (l2:compute l2
          (l4:l4-active-cells l4)        ;; feedforward input (non-bursting)
          (lateral-inputs ccx (patch-l2s patch) l2-prev-actives)
          (l4:l4-predicted-cells l4)     ;; feedforward growth candidates (non-bursting)
          learn
          (l4:l4-predicted-cells l4)))   ;; predicted input (non-bursting)
...
(define (reset-seq patch)
  (vector-for-each  l4:reset-seq  (patch-l4s patch)))
                                                                                            ;
(define (print-statistics patch)
  (l2:print-statistics (patch-l2s patch))
  (l4:print-statistics (patch-l4s patch)))
